...
def _event_from_row(row: dict):
    from ..calendar.store import Event, normalize_availability
...
def create_event(
    ...
    notes: str = "", rrule: str = "", attendees: str = "", availability: str = "busy",
):
    import uuid
    ...
    event = calendar_store.Event(
        ...
        availability=calendar_store.normalize_availability(availability),
        created=now,
        updated=now,
    )
    with connect(settings) as conn:
        conn.execute(
            ...
            "(id, title, start, \"end\", location, notes, rrule, attendees, availability, created, updated) "
            "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
            (
                event.id, event.title, event.start, event.end, event.location,
                event.notes, event.rrule, event.attendees, event.availability,
...
def get_event(settings: Settings, event_id: str):
    ensure_calendar_schema(settings)
    with connect(settings) as conn:
        rows = _rows(conn.execute("SELECT id, title, start, \"end\", location, notes, rrule, exdates, overrides, organizer, attendees, availability, caldav_href, caldav_etag, created, updated FROM assistant_calendar_events WHERE id = %s", (event_id,)))
...
def list_events(settings: Settings):
    ensure_calendar_schema(settings)
    with connect(settings) as conn:
        rows = _rows(conn.execute("SELECT id, title, start, \"end\", location, notes, rrule, exdates, overrides, organizer, attendees, availability, caldav_href, caldav_etag, created, updated FROM assistant_calendar_events"))
...
def update_event(settings: Settings, event_id: str, fields: dict[str, str]):
    ...
    if "availability" in updates:
        updates["availability"] = calendar_store.normalize_availability(updates["availability"])
    if not updates:
        return existing
...
def restore_event(settings: Settings, event) -> object:
    ...
    ensure_calendar_schema(settings)
    event.availability = calendar_store.normalize_availability(event.availability)
    with connect(settings) as conn:
        conn.execute(
            """
            INSERT INTO assistant_calendar_events
              (id, title, start, "end", location, notes, rrule, exdates, overrides, organizer, attendees, availability, caldav_href, caldav_etag, created, updated)
            ...
            """,
            (event.id, event.title, event.start, event.end, event.location, event.notes, event.rrule, event.exdates, event.overrides, event.organizer, event.attendees, event.availability, event.caldav_href, event.caldav_etag, event.created, event.updated),
        )
    return event
...
def find_by_href(settings: Settings, href: str):
    ...
    ensure_calendar_schema(settings)
    with connect(settings) as conn:
        rows = _rows(conn.execute("SELECT id, title, start, \"end\", location, notes, rrule, exdates, overrides, organizer, attendees, availability, caldav_href, caldav_etag, created, updated FROM assistant_calendar_events WHERE caldav_href = %s", (href,)))
    return _event_from_row(rows[0]) if rows else None
