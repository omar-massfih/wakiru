...
def summarize(settings: Settings, habit: str, today: date) -> str:
    ...
    last = entries[0]
    val = f" — {num(last.value)} {last.unit}".rstrip() if last.value else ""
    ...
    if recent:
        ...
        if valued:
            ...
            lines.append(f"  7-day average value: {num(avg)} {recent[0].unit}".rstrip())
    # A few recent entries with ids, so the model can correct a mis-log.
    lines.append("  recent:")
    for e in entries[:5]:
        v = f" {num(e.value)} {e.unit}".rstrip() if e.value else ""
