#!/usr/bin/env python3
"""One-time helper to mint a Google Drive OAuth2 *refresh token* for Wakiru's backup.
...
``.env`` (or the k8s Secret) as ``DRIVE_OAUTH_REFRESH_TOKEN``.

    python scripts/drive_oauth_setup.py \\
        --client-id XXX.apps.googleusercontent.com \\
        --client-secret GOCSPX-...

Reuses the *same* Google Cloud OAuth Desktop-app client you already use for
Calendar/Gmail — but you must mint a new refresh token here, because Google refresh
tokens are scope-bound and the Calendar/Gmail ones don't carry a Drive scope. The
scope requested is ``drive.file`` (least privilege: the app only ever sees files it
created, i.e. its own backups).

Prereqs (Google Cloud Console): the Drive API enabled, an OAuth **Desktop app**
...
def main() -> int:
    ...
    print("\n=== Add these to .env / the k8s Secret ===")
    print("DRIVE_BACKUP_ENABLED=true")
    print(f"DRIVE_OAUTH_CLIENT_ID={args.client_id}")
    print(f"DRIVE_OAUTH_CLIENT_SECRET={args.client_secret}")
    print(f"DRIVE_OAUTH_REFRESH_TOKEN={refresh}")
    return 0
